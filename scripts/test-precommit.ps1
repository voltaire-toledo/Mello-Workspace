...
function Test-RequiresHeader {
    ...
    if ($content -notmatch '#Requires AutoHotkey v2') {
        Write-Verbose "File $FilePath is missing the required header."
        return $false
    }
    Write-Verbose "File $FilePath has the required header."
    return $true
}
    ...
    if ($content -match '[C-Z]:\\' -or $content -match '[C-Z]:/') {
        ...
        foreach ($line in Get-Content $FilePath) {
            ...
            if ($line -match '[C-Z]:\\' -and $line -notmatch '^\s*;') {
                Write-Verbose "Hardcoded path found in $FilePath at line $lineNum."
